...
public class ParkingService {
  ...
  // Assignment 1 will rework this method. Currently it does not handle errors and missing parameters well.
  protected String handleInput(InputStream in) {
    ...
    logger.log(Level.FINE, "data: {0}", String.join(", ",data));
    return performCommand(data.remove(0), data.toArray(String[]::new));
  }

  private String performCommand(String command, String[] args) {
    ...
    switch (command) {
      ...
      case "CHARGES":
			if(checkNumberOfParameters(1, args.length)) {
				String id = args[0];
				Customer c = parkingOffice.getCustomer(id);
				Money charge = parkingOffice.getParkingCharges(c);
